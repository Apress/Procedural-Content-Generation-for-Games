...
from material_and_image_utils import get_context_override, add_material_to_obj, create_material, set_mapping_node_scale, set_viewport_material_preview
from texture_material_generation import create_grip, create_shiny_metal, create_brushed_metal, create_fine_grid_metal
...
def generate_gun(context, name, location=(0, 0, 0), num_cir_segments=32, grip_radius=3, num_grip_levels=6, \
    ...
    for i in range(total_levels):
        ...
        # --------- RECORD THE FACES ON THE SIDE OF THE GRIP, FOR ASSIGNING MATERIAL TO LATER ---------
        if i>0 and i<num_grip_levels:
            for f in bm.faces:
                if f.select:
                    grip_faces.append(f)
        # ---------------------------------------------------------------------------------------------
    ...
    # -------------------- CREATE & ASSIGN MATERIAL TO GRIP & GUN (MINUS BARRELS & TRIGGER) --------------------    
    bpy.ops.mesh.select_all(action='DESELECT')
    for f in grip_faces:
        f.select = True
    bpy.ops.mesh.select_all(action='INVERT')
    bpy.ops.uv.smart_project()
    gun_mat = create_material(grip_obj, grip_obj.name+"_gun_mat")
    ...
    for f in grip_faces:
        f.select = True
    bpy.ops.uv.smart_project()
    grip_mat = create_material(grip_obj, grip_obj.name+"_grip_mat")
    bpy.ops.object.material_slot_assign()
    # ----------------------------------------------------------------------------------------------------------
    ...
    # ---------------------- CREATE & ASSIGN MATERIAL TO BARRELS ------------------------
    bpy.ops.object.select_all(action='DESELECT')
    barrel_obj.select_set(True)
    bpy.ops.object.transform_apply(location=True, rotation=False, scale=False)
    bar_bevel_mod = add_bevel_modifier(barrel_obj, width=2, segments=3)
    bpy.ops.object.modifier_apply(modifier=bar_bevel_mod.name)
    bpy.ops.object.shade_smooth()
    bpy.ops.object.mode_set(mode='EDIT')
    
    bpy.ops.mesh.select_all(action='SELECT')
    bpy.ops.uv.smart_project()
    barrel_mat = create_material(barrel_obj, barrel_obj.name+"_mat")
    barrel_mat.node_tree.nodes["Principled BSDF"].inputs[0].default_value = (0, 1, 0, 1)
    bpy.ops.object.material_slot_assign()
    bpy.ops.object.mode_set(mode='OBJECT')
    # -----------------------------------------------------------------------------------    
    ...
    # ---------------------- CREATE & ASSIGN MATERIAL TO TRIGGER ------------------------
    bpy.ops.object.select_all(action='DESELECT')
    trigger_obj.select_set(True)
    trigger_bevel_mod = add_bevel_modifier(trigger_obj, width=2, segments=3)
    trigger_taper_mod = add_simple_deform_taper_modifier(trigger_obj)
    bpy.ops.object.modifier_apply(modifier=trigger_bevel_mod.name)
    bpy.ops.object.modifier_apply(modifier=trigger_taper_mod.name)
    bpy.ops.object.transform_apply(location=True, rotation=False, scale=False)
    bpy.ops.object.shade_smooth()
    bpy.ops.object.mode_set(mode='EDIT')
    ...
    bpy.ops.mesh.select_all(action='SELECT')
    bpy.ops.uv.smart_project()
    trigger_mat = create_material(trigger_obj, trigger_obj.name+"_mat")
    bpy.ops.object.material_slot_assign()
    bpy.ops.object.mode_set(mode='OBJECT')
    # -----------------------------------------------------------------------------------    
    ...
    # ------------------------- SET VIEWPORT TO MATERIAL PREVIEW, RETURN MATERIALS ----------------------------
    set_viewport_material_preview(context)
    return gun_mat, grip_mat, barrel_mat, trigger_mat, grip_obj, barrel_obj, trigger_obj
    # ---------------------------------------------------------------------------------------------------------
...
#=========================================================================================================================        
def test_gen_guns_with_mats(context):
    # ------------------------- BLACK CROSS GRIP, SILVER ------------------------------------
    normal_map_cross_motif_filepath = script_dir + "/" + "cross_motif_grid_normal_map.png"
    black = (0, 0, 0, 1)
    silver = (0.604, 0.604, 0.604, 1.0)
    
    gun_mat, grip_mat, barrel_mat, trigger_mat, grip_obj, barrel_obj, trigger_obj = \
        generate_gun(context, "0_default_gun", location=(15, 0, 0))
    
    create_grip(grip_obj, grip_mat, black, normal_map_cross_motif_filepath)
    create_brushed_metal(grip_obj, gun_mat, silver)
    create_shiny_metal(barrel_mat, silver)
    create_fine_grid_metal(trigger_obj, trigger_mat, silver)
    # ---------------------------------------------------------------------------------------
    
    # ------------------------- BROWN ROUND BUMP GRIP, GOLD BODY, COPPER BARRELS & TRIGGER -----------------------------
    normal_map_round_bumps_filepath = script_dir + "/" + "round_bumps_normal_map.png"
    brown = (0.086, 0.02, 0.004, 1)
    gold = (1.000, 0.829, 0.359, 1.000)
    copper = (1.000, 0.383, 0.249, 1.000)
    
    gun_mat1, grip_mat1, barrel_mat1, trigger_mat1, grip_obj1, barrel_obj1, trigger_obj1 = \
        generate_gun(context, "1", location=(20, 0, -55), num_cir_segments=56, grip_radius=3, num_grip_levels=8, \
        stylize=True, side_rib_only=False, num_barrels=3)
        
    create_grip(grip_obj1, grip_mat1, brown, normal_map_round_bumps_filepath)
    create_brushed_metal(grip_obj1, gun_mat1, gold)
    create_shiny_metal(barrel_mat1, copper)
    create_fine_grid_metal(trigger_obj1, trigger_mat1, copper)
    # ------------------------------------------------------------------------------------------------------------------
    
    # ------------------------- BROWN CROSS GRIP, SILVER ----------------------------------------------------------
    gun_mat2, grip_mat2, barrel_mat2, trigger_mat2, grip_obj2, barrel_obj2, trigger_obj2 = \
        generate_gun(context, "2", location=(160, 0, -50), num_cir_segments=32, grip_radius=4, num_grip_levels=8, \
        stylize=True, grip_bent_factor=-0.5, side_rib_only=True, num_barrels=1, ratio_frame_to_grip=1.0, \
        slide_back_multiplier=1.8, grip_width_multiplier=2.5)
        
    create_grip(grip_obj2, grip_mat2, brown, normal_map_cross_motif_filepath)
    create_brushed_metal(grip_obj2, gun_mat2, silver)
    create_shiny_metal(barrel_mat2, silver)
    create_fine_grid_metal(trigger_obj2, trigger_mat2, silver)
    # --------------------------------------------------------------------------------------------------------------

    # ------------------------- BLACK ROUND BUMP GRIP, GOLD --------------------------------------------------------
    gun_mat3, grip_mat3, barrel_mat3, trigger_mat3, grip_obj3, barrel_obj3, trigger_obj3 = \
        generate_gun(context, "3", location=(90, 0, -10), num_cir_segments=32, grip_radius=4, num_grip_levels=8, \
        stylize=True, grip_bent_factor=1.5, side_rib_only=True, num_barrels=1, ratio_frame_to_grip=1.0, \
        slide_back_multiplier=0.8, grip_width_multiplier=2.5)
        
    create_grip(grip_obj3, grip_mat3, black, normal_map_round_bumps_filepath)
    create_brushed_metal(grip_obj3, gun_mat3, gold)
    create_shiny_metal(barrel_mat3, gold)
    create_fine_grid_metal(trigger_obj3, trigger_mat3, gold)
    # --------------------------------------------------------------------------------------------------------------
    
    # ------------------------- BLACK CROSS GRIP, COPPER -----------------------------------------------------------
    gun_mat4, grip_mat4, barrel_mat4, trigger_mat4, grip_obj4, barrel_obj4, trigger_obj4 = \
        generate_gun(context, "4", location=(90, 0, -70), num_cir_segments=40, grip_radius=5, num_grip_levels=5, \
        stylize=False, grip_bent_factor=1, side_rib_only=False, num_barrels=3, ratio_frame_to_grip=1, \
        slide_back_multiplier=2.0, grip_width_multiplier=1.6)
        
    create_grip(grip_obj4, grip_mat4, black, normal_map_cross_motif_filepath)
    create_brushed_metal(grip_obj4, gun_mat4, copper)
    create_shiny_metal(barrel_mat4, copper)
    create_fine_grid_metal(trigger_obj4, trigger_mat4, copper)
    # --------------------------------------------------------------------------------------------------------------
    
def test_gun_export_gltf(context):
    normal_map_cross_motif_filepath = script_dir+"/cross_motif_grid_normal_map.png"
    black = (0, 0, 0, 1)
    silver = (0.604, 0.604, 0.604, 1.0)
    
    gun_mat, grip_mat, barrel_mat, trigger_mat, grip_obj, barrel_obj, trigger_obj = \
        generate_gun(context, "0_default_gun", location=(5, 10, 0))
    create_grip(grip_obj, grip_mat, black, normal_map_cross_motif_filepath)
    create_brushed_metal(grip_obj, gun_mat, silver)
    create_shiny_metal(barrel_mat, silver)
    create_fine_grid_metal(trigger_obj, trigger_mat, silver)    
    
    set_length_units(context)
    gun_objs = [grip_obj,barrel_obj,trigger_obj]
    joined_gun = post_process_objs_for_export(context, gun_objs, 0.36)
    joined_gun.name = "test_gun"
    glb_filepath = script_dir+"/"+joined_gun.name+".glb"
    export_gltf(context, glb_filepath, [joined_gun])
    
#=============================================================================================
if __name__ == "__main__":
    test_gun_export_gltf(bpy.context)